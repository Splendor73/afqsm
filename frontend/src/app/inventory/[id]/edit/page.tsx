...
export default function EditPartPage() {
  ...
  // Fetch part data and categories
  useEffect(() => {
    if (!partId) return;
    ...
    async function fetchData() {
      try {
        ...
        // Fetch categories
        const categoriesResponse = await fetch('http://localhost:5017/api/part-categories');
        ...
        // Fetch part details
        const partResponse = await fetch(`http://localhost:5017/api/parts/${partId}`);
        if (!partResponse.ok) {
          throw new Error(`Error: ${partResponse.status}`);
        }
        ...
        if (partData.success) {
          ...
          setPartData({
            ...
            stock: part.stock ? String(part.stock) : '0',
            threshold: part.threshold ? String(part.threshold) : '5',
          ...
          throw new Error(partData.error || 'Failed to fetch part data');
        }
      } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5017/api/parts/${partId}`, {
        method: 'PUT', // Changed back to PUT to match backend implementation
        headers: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link
          href={partId ? `/inventory/${partId}` : "/inventory"}
